...
import { Crown, Sparkles, Check, PartyPopper, Loader2 } from "lucide-react";
...
function PremiumSuccessContent() {
  ...
  return (
    <Card
      className={`max-w-lg w-full backdrop-blur-xl bg-white/10 border-[var(--color-aurora-yellow)]/30 transition-all duration-700 ${showContent ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}
    >
      <CardContent className="pt-8 pb-6 text-center space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-300">
            Thank you for supporting Aurora App! Your subscription is now
            active.
          </p>
        </div>
        ...
        <div className="bg-gradient-to-r from-[var(--color-aurora-purple)]/20 to-[var(--color-aurora-pink)]/20 border border-[var(--color-aurora-purple)]/30 rounded-xl p-4">
          <p className="text-white text-sm">
            💜 Your subscription helps us keep Aurora App running and supports
            our mission to create safer communities for women worldwide.
          </p>
        </div>
...
function LoadingFallback() {
  return (
    <div className="flex items-center justify-center">
      <Loader2 className="w-8 h-8 animate-spin text-[var(--color-aurora-yellow)]" />
    </div>
  );
}
