  ...
  const sortedItems = feedItems
    ? [...feedItems].sort((a: any, b: any) => {
        switch (sortBy) {
          case "hot":
            const aScore =
              (a.upvotes || 0) - (a.downvotes || 0) + (a.commentCount || 0) * 2;
            const bScore =
              (b.upvotes || 0) - (b.downvotes || 0) + (b.commentCount || 0) * 2;
  ...
  if (viewMode === "immersive") {
    return (
      <div className="min-h-screen bg-[var(--background)]">
        {/* Floating centered control bar - easy to reach on all devices */}
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50">
          <div className="flex items-center gap-2 px-3 py-2 rounded-2xl bg-black/70 backdrop-blur-md border border-white/10 shadow-xl">
            ...
            {isMounted && (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/15 hover:bg-white/25 text-white transition-all min-h-[44px]">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gradient-to-r from-[var(--color-aurora-purple)]/30 to-[var(--color-aurora-pink)]/30 hover:from-[var(--color-aurora-purple)]/50 hover:to-[var(--color-aurora-pink)]/50 text-white transition-all min-h-[44px]">
            ...
            {sortedItems
              .filter((item: any) => {
                ...
                if (contentType === "reel")
                  return (
                    item.type === "post" &&
                    (item.postType === "reel" || item.reel || item.reelId)
                  );
                if (contentType === "ai_chat")
                  return item.type === "post" && item.postType === "ai_chat";
                if (contentType === "post")
                  return (
                    item.type === "post" &&
                    ...
                    {isReel && item.reel && (
                      <ReelFeedCard
                        reel={{
                          ...
                          _creationTime:
                            item.reel._creationTime || item._creationTime,
            ...
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-3">
              ...
              <div className="space-y-2">
                {[
                  ...
                  >
                    <span className="text-xs font-bold text-[var(--muted-foreground)] w-4">
                      {idx + 1}
                    </span>
                    <div className="flex-1 min-w-0">
            ...
            <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-3">
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  >
                    <item.icon
                      className={`w-3.5 h-3.5 text-[var(--color-${item.color})]`}
                    />
                    {item.label}
                  </Link>
                ))}
            ...
            <Button
              onClick={() => {
                const text = encodeURIComponent(
                  "Join me on Aurora App - a safe space for women! 💜",
                );
            ...
            <Button
              onClick={() => {
                ...
                const body = encodeURIComponent(
                  `Join me on Aurora: ${window.location.origin}?ref=${userId}`,
                );
