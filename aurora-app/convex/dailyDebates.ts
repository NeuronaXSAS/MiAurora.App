...
export const replaceDebatesForDate = mutation({
  args: {
    date: v.string(),
    debates: v.array(
      v.object({
    ...
    deactivateExisting: v.optional(v.boolean()),
  },
  handler: async (ctx, args) => {
    const existing = await ctx.db
      .query("dailyDebates")
    ...
    if (args.deactivateExisting ?? true) {
    ...
    for (const debate of args.debates) {
      const existingSlot = existing.find((item) => item.slot === debate.slot);
      if (existingSlot) {
        ...
        createdIds.push(existingSlot._id);
        continue;
      }
...
export const getDebatePipelineStatus = query({
  args: {
    date: v.optional(v.string()),
  },
  handler: async (ctx, args) => {
    ...
    const activeDebates = todayDebates.filter((debate) => debate.isActive);
    const latestDebate = [...todayDebates].sort(
      (a, b) => b._creationTime - a._creationTime,
    )[0];

    return {
      date: targetDate,
      totalDebates: todayDebates.length,
      activeDebates: activeDebates.length,
      completeSetReady: activeDebates.length >= 6,
      autoGenerated: activeDebates.filter((debate) => debate.isAutoGenerated).length,
      manual: activeDebates.filter((debate) => !debate.isAutoGenerated).length,
      sources: Array.from(
        new Set(activeDebates.map((debate) => debate.sourceName)),
      ),
      latestCreatedAt: latestDebate?._creationTime || null,
      latestTitles: activeDebates
